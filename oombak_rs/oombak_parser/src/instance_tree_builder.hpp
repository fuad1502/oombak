...
public:
  Exception(oombak_parser_error_t error) { this->error = error; }
  oombak_parser_error_t get_error_t() { return error; }
...
public:
  InstanceTreeBuilder(oombak_parser_instance_t *root_instance,
  ...
  bool has_error() { return error != OOMBAK_PARSER_ERROR_NONE; }

  oombak_parser_error_t get_error() { return error; }
...
private:
  ...
  void visitInstance(const InstanceSymbol &symbol,
  ...
  void set_name(oombak_parser_instance_t *instance,
  ...
  void set_signals(oombak_parser_instance_t *instance,
                   const vector<oombak_parser_signal_t> &signals) {
    instance->signals_len = signals.size();
    instance->signals = (oombak_parser_signal_t *)malloc(
        signals.size() * sizeof(oombak_parser_signal_t));
    for (int i = 0; i < signals.size(); i++) {
      instance->signals[i] = signals[i];
    }
  }

  vector<oombak_parser_instance_t *>
  ...
  void set_child_instances(
      oombak_parser_instance_t *instance,
      const vector<oombak_parser_instance_t *> &child_instances) {
    instance->child_instances_len = child_instances.size();
    instance->child_instances = (oombak_parser_instance_t **)malloc(
        child_instances.size() * sizeof(oombak_parser_instance_t *));
  ...
  void append_with_port_signals(vector<oombak_parser_signal_t> &signals,
                                const InstanceSymbol &symbol) {
    append_with_signals_of_type<PortSymbol>(signals, symbol);
  }

  void append_with_net_signals(vector<oombak_parser_signal_t> &signals,
                               const InstanceSymbol &symbol) {
    append_with_signals_of_type<NetSymbol>(signals, symbol);
  }

  void append_with_var_signals(vector<oombak_parser_signal_t> &signals,
                               const InstanceSymbol &symbol) {
    append_with_signals_of_type<VariableSymbol>(signals, symbol);
  }

  template <typename T>
  void append_with_signals_of_type(vector<oombak_parser_signal_t> &signals,
                                   const InstanceSymbol &symbol) {
  ...
  template <typename T>
  void
  throw_if_unsupported_symbol_type(Scope::specific_symbol_iterator<T> symbol) {
  ...
  get_port_type(Scope::specific_symbol_iterator<PortSymbol> symbol) {
  ...
  static bool
  is_port_with_name_inside(const char *name,
                           const vector<oombak_parser_signal_t> &signals) {
  ...
  static bool is_port(const oombak_parser_signal_t &s) {
    return (s.type == OOMBAK_PARSER_PACKED_ARR_PORT_IN ||
            s.type == OOMBAK_PARSER_PACKED_ARR_PORT_OUT);
  }

  static std::function<bool(oombak_parser_signal_t)>
  port_with_name(const char *name) {
