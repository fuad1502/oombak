...
private:
  ...
  static std::vector<uint32_t> get_words_vec_from(svBitVecVal *out, int nBits);
  static void set_from_words_vec(svBitVecVal *in,
                                 const std::vector<uint32_t> &words, int nBits);
  static void set_signal_mappings(std::map<std::string, Signal> &signalMapping);

  // setters
  static void set_clk(Dut *self, const std::vector<uint32_t> &words);
  static void set_rst_n(Dut *self, const std::vector<uint32_t> &words);
  static void set_in(Dut *self, const std::vector<uint32_t> &words);
  // getters
