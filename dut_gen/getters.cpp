...
pair<vector<uint32_t>, uint64_t> Dut::get_clk(Dut *self) {
  svBit out;
  self->vDut->v_sample_get_clk(&out);
  return {vector<uint32_t>(1, out), 1};
}
pair<vector<uint32_t>, uint64_t> Dut::get_rst_n(Dut *self) {
  svBit out;
  self->vDut->v_sample_get_rst_n(&out);
  return {vector<uint32_t>(1, out), 1};
}
pair<vector<uint32_t>, uint64_t> Dut::get_in(Dut *self) {
  int nBits = 6;
